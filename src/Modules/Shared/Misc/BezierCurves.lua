	...
	if numPoints > 0 then
		...
		if #points > 1 then
			...
			if lengthIndeces[nearestParameterIndex - 1] then
				p0, p1 =
					self:CalculatePositionAt(lengthIndeces[nearestParameterIndex - 1][1]),
					self:CalculatePositionAt(nearestParameter[1])
			else
				p0, p1 =
					self:CalculatePositionAt(nearestParameter[1]),
					self:CalculatePositionAt(lengthIndeces[nearestParameterIndex + 1][1])
...
function Bezier:CalculateDerivativeRelativeToLength(t: number): Vector3
	...
	if numPoints > 1 then
		...
		if lengthIndeces[nearestParameterIndex - 1] then
			d0, d1 =
				self:CalculateDerivativeAt(lengthIndeces[nearestParameterIndex - 1][1]),
				self:CalculateDerivativeAt(nearestParameter[1])
		else
			d0, d1 =
				self:CalculateDerivativeAt(nearestParameter[1]),
				self:CalculateDerivativeAt(lengthIndeces[nearestParameterIndex + 1][1])
...
function Bezier:CreateVector3Tween(
	object: Instance | { [any]: any },
	propertyTable: { any },
	bezierTweenInfo: TweenInfo,
	...
	if success and foundProperties then
				...
				if playbackState == Enum.PlaybackState.Playing then
					numValueChangedConnection = numValue.Changed:Connect(function(t)
						for _, propName in pairs(propertyTable) do
							object[propName] = RelativeToLength and self:CalculatePositionRelativeToLength(t)
								or self:CalculatePositionAt(t)
						end
					end)
				else
		...
		error(
			"Bezier:CreateVector3Tween() requires a matching property table with Vector3 or nil property names for the object as the second argument!"
		)
	end
end
...
-- works for any object that has the properties given by the property table
function Bezier:CreateCFrameTween(
	object: Instance | { [any]: any },
	propertyTable: { any },
	bezierTweenInfo: TweenInfo,
	RelativeToLength: boolean?
	...
	if success and foundProperties then
		...
		tween.Changed:Connect(function(prop)
			if prop == "PlaybackState" then
				...
				if playbackState == Enum.PlaybackState.Playing then
					numValueChangedConnection = numValue.Changed:Connect(function(t)
						for _, propName in pairs(propertyTable) do
							local position = RelativeToLength and self:CalculatePositionRelativeToLength(t)
								or self:CalculatePositionAt(t)
							local derivative = RelativeToLength and self:CalculateDerivativeRelativeToLength(t)
								or self:CalculateDerivativeAt(t)
		...
		error(
			"Bezier:CreateCFrameTween() requires a matching property table with CFrame or nil property names for the object as the second argument!"
		)
	end
end
